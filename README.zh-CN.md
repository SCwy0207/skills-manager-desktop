...
Skills Manager 将 Codex 本地会话检索和多 Agent Skills 管理整合进一个原生桌面工作区。应用基于 Tauri、Rust 与 React 构建，索引和操作数据保存在本机；扫描、预览和静态审计不会执行 Skill 内容。

> 项目状态：`v1.0.0` 是首个公开版本。当前提供 Windows 安装包；macOS 与 Linux 可从源码构建，但暂未提供官方二进制下载。
...
- 增量索引 Codex 本地 JSONL 会话，支持中英文标题及正文子串检索和高亮。
